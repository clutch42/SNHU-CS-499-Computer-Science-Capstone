...
#define BASE_SIZE 10
#define PRISM_RADIUS 1
#define PRISM_HEIGHT 1
#define PRISM_SIDES 20
#define TORUS_DIST_FROM_CENTER 2.5
#define TORUS_RADIUS 0.5
#define TORUS_SIDES 20
...
int main(int argc, char* argv[])
{
    ...
    Prism prism1 = Prism(PRISM_RADIUS, PRISM_HEIGHT, PRISM_SIDES);
    Base base = Base(BASE_SIZE);
    Cube cube = Cube();
    Torus torus = Torus(TORUS_DIST_FROM_CENTER, TORUS_RADIUS, TORUS_SIDES);
    Sphere sphere = Sphere(SPHERE_RADIUS, SPHERE_SIDES);

    GLMesh cylinderTopMesh;
    cylinderTopMesh.vertsSize = prism1.GetTopVerticesSize();
    cylinderTopMesh.nIndices = prism1.GetTopIndicesSize();
    UCreateMesh(cylinderTopMesh, prism1.GetTopVertices(), prism1.GetTopIndices()); // Calls the function to create the Vertex Buffer Object
    meshs.push_back(cylinderTopMesh);

    GLMesh cylinderSideMesh;
    cylinderSideMesh.vertsSize = prism1.GetSideVerticesSize();
    cylinderSideMesh.nIndices = prism1.GetSideIndicesSize();
    UCreateMesh(cylinderSideMesh, prism1.GetSideVertices(), prism1.GetSideIndices()); // Calls the function to create the Vertex Buffer Object
    meshs.push_back(cylinderSideMesh);

    GLMesh baseMesh;
    baseMesh.vertsSize = base.GetBaseVerticesSize();
    baseMesh.nIndices = base.GetBaseIndicesSize();
    UCreateMesh(baseMesh, base.GetBaseVertices(), base.GetBaseIndices());
    meshs.push_back(baseMesh);

    GLMesh cubeSideMesh;
    cubeSideMesh.vertsSize = cube.GetCubeSideVerticesSize();
    cubeSideMesh.nIndices = cube.GetCubeSideIndicesSize();
    UCreateMesh(cubeSideMesh, cube.GetCubeSideVertices(), cube.GetCubeSideIndices());
    meshs.push_back(cubeSideMesh);

    GLMesh cubeMainMesh;
    cubeMainMesh.vertsSize = cube.GetCubeMainVerticesSize();
    cubeMainMesh.nIndices = cube.GetCubeMainIndicesSize();
    UCreateMesh(cubeMainMesh, cube.GetCubeMainVertices(), cube.GetCubeMainIndices());
    meshs.push_back(cubeMainMesh);

    GLMesh torusMesh;
    torusMesh.vertsSize = torus.GetTorusVerticesSize();
    torusMesh.nIndices = torus.GetTorusIndicesSize();
    UCreateMesh(torusMesh, torus.GetTorusVertices(), torus.GetTorusIndices());
    meshs.push_back(torusMesh);

    GLMesh sphereMesh;
    sphereMesh.vertsSize = sphere.GetSphereVerticesSize();
    sphereMesh.nIndices = sphere.GetSphereIndicesSize();
    UCreateMesh(sphereMesh, sphere.GetSphereVertices(), sphere.GetSphereIndices());
